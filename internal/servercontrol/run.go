...
func (h *Handler) Run(ctx context.Context, genericTask *rctypes.Task[any, any], l *logrus.Logger) error {
	...
	case model.RunOutofband:
		...
		if err := oh.run(ctx, task); err != nil {
			ctxLogger.WithError(err).Error("run() returned error")
			return err
		}
	}
...
func (h *Handler) initTask(ctx context.Context, genericTask *rctypes.Task[any, any], l *logrus.Logger) (*model.ServerControlTask, model.RunMode, *logrus.Entry, error) {
	// prepare new logger for handler
	logger := logrus.New()
	logger.Formatter = l.Formatter
	logger.Level = l.Level

	task, err := model.CopyAsServerControlTask(genericTask)
	if err != nil {
