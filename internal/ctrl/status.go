...
func (n *NatsController) NewNatsConditionStatusQueryor(conditionKind condition.Kind) (*NatsConditionStatusQueryor, error) {
	errKV := errors.New("unable to connect to status KV for condition progress lookup")
	kvHandle, err := events.AsNatsJetStreamContext(n.stream.(*events.NatsJetstream)).KeyValue(string(conditionKind))
	if err != nil {
