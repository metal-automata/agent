...
func (n *NatsController) processConditionFromEvent(ctx context.Context, msg events.Message, eventAcknowleger eventStatusAcknowleger) {
	...
	if !slices.Contains(n.conditionKinds, cond.Kind) {
		n.logger.WithFields(
			logrus.Fields{
				"subject": msg.Subject(),
				"kind":    cond.Kind,
			},
		).Warn("controller does not support this event kind")
		spanEvent(span, cond, n.ID(), "sent nack, controller does not support this event kind")
		metricsEventsCounter(false, "nack")
		eventAcknowleger.nak()
	...
	conditionStatusQueryor, err := n.NewNatsConditionStatusQueryor(cond.Kind)
	if err != nil {
		eventAcknowleger.nak()
